...
--	dataQuery varchar := 'WITH JsonData AS (SELECT '|| selectClause ||' FROM "EntityType"."EntityType" WHERE "Id">0 ORDER BY "Id" DESC LIMIT 3 OFFSET 0) SELECT JSON_AGG(JsonData.*) FROM JsonData;';
	dataQuery varchar := 'WITH JsonData AS (SELECT '|| selectClause ||' FROM "' || entitytypename || '"."' || entitytypename || '" WHERE '|| whereClause ||' ORDER BY '|| sortClause ||' LIMIT '|| pageSize ||' OFFSET '|| pageNumber ||') SELECT JSON_AGG(JsonData.*) FROM JsonData;';
